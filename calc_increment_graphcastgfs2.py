...
#filename_gcast = "graphcastgfs.t00z.pgrb2.0p25.f024"
#filename_gfs = "gfs.t00z.pgrb2.0p25.f024"
#filename_inc = 'graphcast_inc.nc'
...
gh2, plevs2 = read_array(grbs,'gh', levs=plevs)
g = 9.8066
z2 = gh2/g
...
print('z_inc min/max',z_inc.min(),z_inc.max())

def getmean(a,coslats):
    return (a*coslats).mean()/coslats.mean()
coslats = np.cos(np.radians(lats))
#print(mslp.min(), mslp.max(),getmean(mslp,coslats))
#print(mslp2.min(), mslp2.max(),getmean(mslp2,coslats))

mslp_inc = mslp-mslp2
print('mslp_inc min/max',mslp_inc.min(), mslp_inc.max())
...
below2, above2, slope2 = get_indices_slope(press_target2, press_data)
...
z_inc_out = vert_interp(z_inc, below2, above2, slope2)
...
delz_inc = nc.createVariable('delz_inc',np.float32,('lev','lat','lon'),zlib=zlib)
...
tmp_inc[:] = inc[:,::-1,:]
delp_inc_out = np.zeros(u_inc_out.shape, u_inc_out.dtype)
## assume ps_inc = mslp_inc, infer delp_inc?
#for k in range(nlevsout):
#    delp_inc_out[k] = mslp_inc*(bk[k]-bk[k+1]) 
inc = taper_vert*delp_inc_out[:]
print('delp increment min/max',inc.min(), inc.max())
delp_inc[:] = inc[:,::-1,:]
inc = np.zeros(z_inc_out.shape, z_inc_out.dtype)
for k in range(nlevsout):
    if k==nlevsout-1:
        inc[k] = z_inc_out[k]
    else:
        inc[k] = z_inc_out[k]-z_inc_out[k+1]
    inc[k] = taper_vert[k]*inc[k]
print('delz increment min/max',inc.min(), inc.max())
delz_inc[:] = inc[:,::-1,:]
inc = taper_vert*q_inc_out[:]
if zero_qinc:
    inc[:] = 0 # zero out humidity increment
...
#fig=plt.figure()
#plt.subplot(2,1,1)
#plt.imshow(delz_inc[nlevsout-1],cmap='bwr',vmin=-25,vmax=25)
#plt.title('delz_inc')
#plt.subplot(2,1,2)
#plt.imshow(delp_inc[nlevsout-1],cmap='bwr',vmin=-25,vmax=25)
#plt.title('delp_inc')
#plt.savefig('inc.png')
nc.close()
